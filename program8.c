...
// DFS Traversal (Modified to include all nodes)
void DFS(struct Graph* graph, int startVertex) {
    bool visited[MAX_VERTICES] = { false };
    printf("DFS starting from vertex %d: ", startVertex);
    DFSUtil(graph, startVertex, visited);

    // Check for any unvisited vertices (to handle disconnected graphs)
...
// BFS (Breadth First Search) (Modified to include all nodes)
...
// Topological Sort - Using DFS (Modified to include all nodes)
...
// Topological Sort (Modified to include all nodes)
...
void displayAdjMatrix(struct Graph* graph) {
    ...
    // Create an adjacency matrix and initialize to 0
    ...
    for (int i = 0; i < graph->numVertices; i++) {
        struct Node* adjList = graph->adjList[i];
        while (adjList != NULL) {
            adjMatrix[i][adjList->vertex] = 1;  // Mark edge from i to adjVertex
            adjList = adjList->next;
        }
    }
